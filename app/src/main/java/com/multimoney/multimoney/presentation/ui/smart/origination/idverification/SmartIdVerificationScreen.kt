...
@Composable
@Preview
fun SmartIdVerificationScreen(
    sharedViewModel: SignUpViewModel = hiltViewModel(),
    viewModel: SmartIdVerificationViewModel = hiltViewModel()
) {
    val context = LocalContext.current
    val launchOnFidoActivityResult =
        rememberLauncherForActivityResult(StartActivityForResult()) { result ->
            viewModel.onUIEvent(
                SmartIdVerificationViewModel.UIEvent.OnOpenOnFidoSdk(
                    result,
                    onOnFidoCompleted = {
    ...
    viewModel.onUIEvent(
        SmartIdVerificationViewModel.UIEvent.OnInitValues(
            false,
                            ...
                            launchOnFidoActivityResult.launch(
                                viewModel.onFidoHelper.getOnFidoIntent(
                                    ...
                                    onRefreshToke = { refreshToken ->
                                        viewModel.onUIEvent(
                                            SmartIdVerificationViewModel.UIEvent.RefreshOnFidoToken(
                                                sharedViewModel.idBrand,
    ...
    LaunchedEffect(true) {
        viewModel.onUIEvent(
            SmartIdVerificationViewModel.UIEvent.OnCallInFidoToken(
                sharedViewModel.idBrand,
                sharedViewModel.userData,
                context.packageName
            )
