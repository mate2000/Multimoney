...
class ProductViewModel @Inject constructor(
    ...
    private fun onSaveFirebaseToken() {
        if (uiState.idBrand != ID_BRAND_EMPTY) {
    ...
    private fun onNavigateToVisaActivateScreen() = navigateTo(
        Screen.VisaIssuanceScreen.baseRoute
            ...
            .plus(getNavParam(APPLY_COMMERCE, balanceCredit?.getFirstSummary()?.applyCommerce ?: false))
    )

    private fun onNavigateToHomeMultimoneyVisa(isForPayment: Boolean = false) {
        ...
        navigateTo(
            Screen.VisaCardScreen.baseRoute
                ...
                .plus(getNavParam(ID_LOAN_CLIENT, uiState.userStatus?.infoCredit?.idLoanClient ?: 0))
                .plus(getNavParam(APPLY_COMMERCE, balanceCredit?.getFirstSummary()?.applyCommerce ?: false))
                .plus(getNavParam(IS_PAYMENT_MODE, isForPayment))
        )
    }
    ...
    private fun onQuickActionClicked(
        ...
        when (flow) {
            ...
            QuickActionFlow.PAY_AT_BUSINESS.flow -> onNavigateToHomeMultimoneyVisa(isForPayment = true)
    ...
    private fun isSmartCtaBlocked() {
        if (balanceCredit?.balanceAccountSmart.isNullOrEmpty().not()) {
            uiState = uiState.copy(
                isCtaBlocked = !balanceCredit?.balanceAccountSmart?.get(
    ...
    companion object {
        ...
        private const val ID_BRAND_EMPTY = "0"
    }
}
