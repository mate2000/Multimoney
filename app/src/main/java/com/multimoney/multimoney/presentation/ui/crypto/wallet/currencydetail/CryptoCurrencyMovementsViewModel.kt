...
class CryptoCurrencyMovementsViewModel @Inject constructor(
    ...
    private fun onNavigateToSelectAccount() {
        navigateTo(
            "${Screen.PurchaseCryptoFlow.baseRoute}?$ITEM_CRYPTO_MARKET=${
                encodeData(
                    MarketCryptoCoin(
    ...
    private fun onNavigateToReleaseTransaction(cryptoItem: CryptoCurrencyMovement?) {
        navigateTo("${Screen.ReleaseTransactionScreen.baseRoute}/${cryptoItem?.market}/${cryptoItem?.id}/${Screen.CryptoCurrencyMovementsScreen.baseRoute}")
    }
