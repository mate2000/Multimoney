...
fun CryptoReceiveCurrenciesListScreen(
    ...
    LaunchedEffect(key1 = true) {
        ...
        viewModel.onUIEvent(UIEvent.OnSetOpenMaintenanceAction(
            action = {
...
fun CryptoReceiveContent(
    ...
    val crListOfCryptoCoin = viewModel.uiState.availableCryptoCoins?.availableCryptoCoins ?: emptyList()
    val svListOfCryptoCoins = viewModel.uiState.availableCryptoCoins
        ?.availableCryptoCoins?.filter { it.baseAsset == CryptoConstants.BTC } ?: emptyList()
...
@Composable
fun CryptoReceiveHeader() {
    Row(
        modifier = Modifier.fillMaxWidth(),
        horizontalArrangement = Arrangement.Start
    ) {
        Text(
            text = stringResource(id = R.string.crypt_list_receive_title),
            style = Typography.h5,
...
@Composable
fun FavoriteSection() {
    val imageChipState = remember { mutableStateOf(false) }

    CustomImageChip(
        iconId = R.drawable.ic_chip_star_outline,
        isSelected = imageChipState.value,
        onChecked = { imageChipState.value = it },
        background = MultimoneyTheme.colors.creditDetailBackground,
        tintColor = MultimoneyTheme.colors.text
    )
}
...
fun AllSection() {
    CustomInformativeChip(
        ...
        text = stringResource(id = R.string.crypt_all_chip_label),
...
fun FilterBottomSheet(
    ...
    Column(
        ...
        LazyColumn(
            ...
            items(com.multimoney.multimoney.presentation.ui.crypto.market.MarketFilter.values()) { filter ->
                CustomSelector(
                    text = filter.value,
            ...
            cryptoCoin.amountchange?.let {
                MarketCurrencyItem(
                    ...
                    percentChange = stringResource(
                        ...
                        cryptoCoin.percentChange?.toDouble()
